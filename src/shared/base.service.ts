...
import { ModelType, InstanceType, Typegoose } from 'typegoose'
...
export abstract class BaseService<T extends Typegoose> {
  ...
  async delete(id: string): Promise<InstanceType<T>> {
    return this._model.findByIdAndRemove(this.toObjectId(id))
  }

  async update(id: string, item: InstanceType<T>) {
